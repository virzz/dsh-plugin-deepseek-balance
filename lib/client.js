window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".dsb-period{flex:none;display:inline-flex;align-items:center;height:18px;padding:0 6px;border:.5px solid currentColor;border-radius:6px;font-size:11px;line-height:1;letter-spacing:.02em;opacity:.85}",
			'.dsb-row[data-period="off"] .dsb-period{color:var(--dsw-alias-state-success-primary)}',
			'.dsb-row[data-period="peak"] .dsb-period{color:var(--dsw-alias-state-warn-primary)}',
			...
			".dsb-row.dsb-rail .dsb-label,.dsb-row.dsb-rail .dsb-value,.dsb-row.dsb-rail .dsb-period{display:none}",
			".dsb-railGlyph{color:var(--dsw-alias-label-secondary);font-size:13px;line-height:1}",
			'.dsb-row.dsb-rail[data-period="off"] .dsb-railGlyph{color:var(--dsw-alias-state-success-primary)}',
			'.dsb-row.dsb-rail[data-period="peak"] .dsb-railGlyph{color:var(--dsw-alias-state-warn-primary)}',
		].join("");
		...
		/**
		 * Peak pricing windows as minutes after midnight, Beijing time. DeepSeek charges half
		 * price outside them. Beijing is UTC+8 with no daylight saving, so a fixed shift keeps
		 * this correct no matter which timezone the browser happens to be in.
		 */
		const BEIJING_SHIFT_MS = 8 * 60 * 60 * 1000;
		...
		/**
		 * Which tariff applies at an instant: "peak" inside a weekday peak window, else "off".
		 *
		 * @param ms - epoch milliseconds.
		 * @returns the tariff key.
		 */
		function periodOf(ms) {
			const shifted = ms + BEIJING_SHIFT_MS;
			const day = Math.floor(shifted / DAY_MS);
			const minute = Math.floor((shifted - day * DAY_MS) / 60000);
			const weekday = (day + 4) % 7; // 1970-01-01 was a Thursday, so 0 = Sunday here
			if (weekday < 1 || weekday > 5) return "off";
			for (let index = 0; index < PEAK_WINDOWS.length; index += 1) {
				if (minute >= PEAK_WINDOWS[index][0] && minute < PEAK_WINDOWS[index][1]) return "peak";
			}
			return "off";
		}

		/** Whole minutes until the tariff flips. The longest gap is Fri 18:00 → Mon 09:00. */
		function minutesUntilSwitch(ms) {
			const current = periodOf(ms);
			const limit = 4 * 24 * 60;
			for (let step = 1; step <= limit; step += 1) {
				if (periodOf(ms + step * 60000) !== current) return step;
			}
			return 0;
		}
		...
		/**
		 * The balances worth showing. A currency whose total is zero or negative carries no
		 * information for a balance read-out, so it is left out of both the row and its
		 * tooltip — and only counted, so the tooltip can say that something was hidden.
		 */
		function positiveOf(balances) {
			const shown = [];
			for (let index = 0; index < balances.length; index += 1) {
				const value = Number(balances[index].total);
				if (Number.isFinite(value) && value > 0) shown.push(balances[index]);
			}
			return shown;
		}
		...
		function BalanceRow(props) {
			...
			React.useEffect(() => {
				load();
				setNow(Date.now());
				// The tariff flips on wall-clock time rather than on a fetch, so the tick drives
				// both the clock the period is read from and the snapshot re-read.
			...
			const shown = positiveOf(balances);
			const hidden = balances.length - shown.length;
			const period = periodOf(now);
			const periodShort = period === "peak" ? "高峰" : "空闲";
			const periodPrice = period === "peak" ? "全价" : "½价";
			const periodAdvice = period === "peak" ? "高峰时段建议节约蹬" : "空闲时段随便蹬";
			const untilSwitch = minutesUntilSwitch(now);
			const periodLine = periodShort + "时段 · " + periodPrice + " · " + periodAdvice
				+ (untilSwitch > 0 ? "（距切换还有 " + humanMinutes(untilSwitch) + "）" : "");
			let tone = "loading";
			...
			if (state.phase === "ok") {
				...
				value = shown.length > 0 ? shown.slice(0, 2).map(amountOf).join(" · ") : "0";
				const lines = ["DeepSeek 余额（常驻 node 采集）", periodLine];
				for (let index = 0; index < shown.length; index += 1) {
					const entry = shown[index];
					lines.push(entry.currency + " " + entry.total + "（赠金 " + entry.granted + " · 充值 " + entry.toppedUp + "）");
				}
				if (shown.length === 0) lines.push("所有币种余额均不大于 0")
				else if (hidden > 0) lines.push("已隐藏 " + hidden + " 项非正余额")
			...
			if (wide !== true) {
				const primary = shown.length > 0 ? shown[0] : undefined;
				const glyph = state.phase === "ok"
					? (primary === undefined ? "0" : symbolOf(primary.currency))
					: state.phase === "error" ? "!" : "…";
			...
			return React.createElement("button", {
				...
				React.createElement("span", { key: "period", className: "dsb-period" }, periodShort),
